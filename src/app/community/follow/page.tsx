...
const usePosts = (userId: number | null) => {
  ...
  const loadDraftPosts = useCallback(async () => {
    ...
    try {
      ...
      // 🔥 PostResponse를 Post 타입으로 변환 (User ID 추가)
      const convertedDrafts: Post[] = draftPosts.map(post => ({
        ...
        userId: (post as any).user?.id || post.author.id // 🔥 실제 User ID 저장
      }))
...
export default function WritePage() {
  ...
  // 🔥 수정: handleSavePost 함수 - 실제 User ID 사용
  const handleSavePost = useCallback(async (status: "draft" | "published") => {
    ...
    // 🔥 실제 User ID 사용 (getCurrentUserId()에서 반환된 값)
    const postData: CreatePostData = {
      ...
      author: { id: userId } // 🔥 실제 User ID 전달
    }

    try {
      if (editingPost) {
        // 🔥 수정 모드 - 실제 User ID 사용
        const actualUserId = editingPost.userId || userId;
        await updatePost(editingPost.id, actualUserId, postData)
        alert("글이 수정되었습니다!")
      } else {
        // 생성 모드
  ...
  // 🔥 수정: handleToggleStatus 함수 - 실제 User ID 사용
  const handleToggleStatus = useCallback(async (postToToggle: Post) => {
    ...
    try {
      ...
      // 🔥 실제 User ID 사용
      const actualUserId = postToToggle.userId || userId;
      await updatePost(postToToggle.id, actualUserId, updateData)
  ...
  // 🔥 수정: handleDeletePost 함수 - 실제 User ID 사용
  const handleDeletePost = useCallback(async (postToDelete: Post) => {
    ...
    try {
      // 🔥 실제 User ID 사용
      const actualUserId = postToDelete.userId || userId;
      await deletePost(postToDelete.id, actualUserId)
